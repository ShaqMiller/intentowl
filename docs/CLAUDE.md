...
- One commit per passing milestone minimum. Conventional commits (`feat:`,
  `fix:`, `chore:`).
- Current milestone: **M7 in progress** (M0-M6 done). Dashboard, Supabase auth
  and the deploy config are built; auth is verified end to end. Not yet
  deployed anywhere. M3 and M4 exit tests were demonstrated but not run to
  their full duration; see Open items.

## Open items carried forward

- **Supabase auth is live and verified** (2026-09-10). Sign-up, the
  confirmation link, `/auth/callback` and the account claim were all exercised
  against the real project: `customers.auth_user_id` is populated and
  `/dashboard` redirects to `/login` without a session. Only `SUPABASE_URL`
  and `SUPABASE_ANON_KEY` are needed — no service-role key, because "sign out
  everywhere" uses `scope: "others"` on the user's own session.
  **Email confirmations must stay ON**: claiming a customer row is authorised
  by proving control of its email address, so with confirmations off anyone
  knowing a customer's email could take their account.

- **Not deployed.** See `docs/DEPLOY.md`. Domain, Cloudflare Email Routing
  (`tools@intentowl.com`) and Resend on `send.intentowl.com` are done; Vercel,
  Railway, DMARC, Supabase SMTP and live Stripe are not.
